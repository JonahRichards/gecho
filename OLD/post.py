...
# PARSE AND PLOT THE DATA FROM THE FIELD MONITOR

def parse_mon():
    res = {}
    with open(f"{DAT_DIR}/Monitor_m00_N01.txt") as fp:
        for i in range(4):
    ...
    for k, v, in res.items():
        try:
            ...
            pass
    res["k_r"], res["k_z"] = int(res["k_r"]), int(res["k_z"])
    return res

p = parse_mon()
dim=p["k_r"]*p["k_z"] + 1

df = pd.read_csv(f"{DAT_DIR}/Monitor_m00_N01.txt", skiprows=4, header=0, names=range(dim+1), sep=" ")
...
def plot_field():
    ...
    plt.savefig(f"{IMG_DIR}/field.png", pad_inches=0.1, bbox_inches="tight")
    plt.close()
    ...
    plt.savefig(f"{IMG_DIR}/power.png", pad_inches=0.1, bbox_inches="tight")
    plt.close()
plot_field()
...
spc, frq_spc, tim_spc, _ = plt.specgram(E, Fs=fs, NFFT=nfft, noverlap=novl, xextent=(t[0], t[-1]))#, #cmap="magma") #"gnuplot2_r"
